...
{
  size_t len;
  char *body, *header, *x, *y;

  if(dictionary_has_key(query, "x") && dictionary_has_key(query, "y")){
    x = dictionary_get(query, "x");
    y = dictionary_get(query, "y");
    Sleep(10);
    body = strdup(append_strings(to_string(atoi(x) + atoi(y)), "\n"));
  }
  else {
    body = strdup("Please provide two numbers\n");  
  }
